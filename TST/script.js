...
function fillFiles(inputText) {
    fs.readFile('templates/schema.json', 'utf8', (err, data) => {
        ...
        createFile(`${inputText}/content-types/${inputText}/schema.json`, newData, () => {
    ...
    fs.readFile('templates/documentation.json', 'utf8', (err, data) => {
        ...
        const newData = data.replace(/abodyyy/g, inputText.toLowerCase()).replace(/Abodyyy/g, inputText.charAt(0).toUpperCase() + inputText.slice(1));
        createFile(`${inputText}/documentation/1.0.0/${inputText}.json`, newData, () => {
            console.log(`File "schema.json" created successfully in "content-type/${inputText}" folder!`);
        });
    });
